 ...
 * 依赖libs目录下的armeabi目录中的libmp3lame.so 和flame.jar<br/>
 * 
 * 权限要求<br/>
 * 
...
public class AudioRecorder2Mp3Util {
	...
	/**
	 * 采样频率:44100Hz是当前唯一能保证在所有设备上工作的采样率，在一些设备上还有22050, 16000或11025。
	 */
	 ...
	 * @param mp3Path
	 */
	public AudioRecorder2Mp3Util(Context context, String rawPath,
								 String mp3Path) {
		this.context = context;
	...
	private void getFilePath() {
		try {
			...
			if (rawPath == null) {
				File raw = new File(context.getDir(folder,
						context.MODE_PRIVATE), fileName + ".raw");
				raw.createNewFile();
				rawPath = raw.getAbsolutePath();
				raw = null;
			}
			if (mp3Path == null) {
				File mp3 = new File(context.getDir(folder,
						context.MODE_PRIVATE), fileName + ".mp3");
				mp3.createNewFile();
...
//								mBuffer.length);
//						long v = 0;// r是实际读取的数据长度，一般而言r会小于buffersize
//						
//						for (int i = 0; i < readSize; i++) {
//							output.writeShort(mBuffer[i]);
//							v += mBuffer[i] * mBuffer[i];//获取音量的数据。
//						}
//						
//						double mean = v / (double) readSize;
//						volume = 10 * Math.log10(mean);
//						LogUtil.i("volumeLevel", volume +"+++++++++++++++");
//						// 大概一秒十次
//						synchronized (mLock) {
//							try {
//								mLock.wait(100);
...
//		}
	...
	class Sample {
		...
		void start() {
			// util=new AudioRecorder2Mp3Util(context); 一般是用这个
			if (util == null)
